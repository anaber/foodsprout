<link rel="stylesheet" href="<?php echo base_url()?>css/photos.css" type="text/css" media="screen" />

<script src="<?php echo base_url()?>js/jquery.lightbox-0.5.js" type="text/javascript"></script>
<script src="<?php echo base_url()?>js/jquery.colorize.js" type="text/javascript"></script>
<script src="<?php echo base_url()?>js/admin/photos_search_admin.js" type="text/javascript"></script>
<script src="<?php echo base_url()?>js/admin/admin_ajax_page_common_content.js" type="text/javascript"></script>

<?php $this->load->view('admincp/includes/ajax_paging'); ?>